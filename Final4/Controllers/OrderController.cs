...
namespace Final4.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        [HttpGet]
        [Route("GetAllOrderByUserId/{id}")]
        public async Task<IActionResult> GetAllOrderByUserId(int id)
        {
            Console.WriteLine("chay dc controller");
            if (id <= 0)
                return BadRequest("Id không hợp lệ");
            Console.WriteLine("van chay dc controller");

            var orders = _orderService.GetAllOrderByUserIdAsync(id);
            return Ok(orders);
        }
    }
}
